...
\textbf{Sect.~\ref{sec:background:families}}~\cite{ref_36}. Two properties
...
either splits the sparse one or merges the dense ones~\cite{ref_37}, and
...
definitions follow~\cite{ref_36}:
...
\cite{ref_37} recommends at least $n + 1$ for $n$ features and larger for
...
$\mathcal{O}(m^2)$ of computing every pair~\cite{ref_37}. Space is
...
%    ref_36   Ester, Kriegel, Sander & Xu (1996), A density-based
%             algorithm for discovering clusters in large spatial
%             databases with noise
%                                cited in: Overview, Formulation
%    ref_37   Schubert et al. (2017), DBSCAN revisited, revisited
